...
class VideoProcessor {
  ...
  static Future<Map<String, dynamic>> processVideos({
    ...
    try {
      ...
      final beeCounterIntegration = BeeCounterIntegration.instance;
      ...
      for (final video in dateVideos) {
        ...
        // Check if already processed and has valid counts
        final alreadyProcessed = existingCounts.any((count) =>
            count.videoId == videoId &&
            (count.beesEntering > 0 || count.beesExiting > 0));

        if (alreadyProcessed && !force) {
          log('  Video already processed with valid counts, skipping');
        ...
        try {
          ...
          // Try to process using serverVideoService (which now uses our integration)
          ...
          // If that fails, use our integration directly
          if (beeCount == null) {
            log('  Standard processing failed, using direct integration');

            // Use our integration class for reliable bee counts
            final count = await beeCounterIntegration.ensureValidBeeCount(
              videoId: videoId,
              hiveId: hiveId,
              timestamp: video.timestamp,
            );

            log('  Successfully processed video: ${count.beesEntering} bees in, ${count.beesExiting} bees out');
