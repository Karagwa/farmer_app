...
class TimeBasedReturnRateDatabase {
  ...
  // Read trip duration distributions for a specific hive within a date range
  Future<List<Map<String, dynamic>>> readTripDurationDistributionsByDateRange(
    String hiveId,
    DateTime startDate,
    DateTime endDate,
  ) async {
    ...
    try {
      ...
      print('Error reading trip duration distributions by date range: $e');
      return [];
    }
  }

  // Delete time block analysis for a specific hive
  Future<int> deleteTimeBlockAnalysisByHiveId(String hiveId) async {
    final db = await instance.database;
    ...
    try {
      ...
      print('Error deleting time block analysis: $e');
      return 0;
    }
  }

  // Delete trip duration distributions for a specific hive
  Future<int> deleteTripDurationDistributionsByHiveId(String hiveId) async {
    final db = await instance.database;
    ...
    try {
      ...
      print('Error deleting trip duration distributions: $e');
      return 0;
    }
  }

  // Delete all data older than a certain date
  Future<int> deleteOldData(DateTime cutoffDate) async {
    final db = await instance.database;

    final cutoffDateStr = DateFormat('yyyy-MM-dd').format(cutoffDate);

    try {
      int timeBlockDeleted = await db.delete(
        'time_block_analysis',
        where: 'date &lt; ?',
        whereArgs: [cutoffDateStr],
      );

      int tripDurationDeleted = await db.delete(
        'trip_duration_distribution',
        where: 'date &lt; ?',
        whereArgs: [cutoffDateStr],
      );

      return timeBlockDeleted + tripDurationDeleted;
    } catch (e) {
